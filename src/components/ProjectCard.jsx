  ...
  return (
    <article className="group flex flex-col rounded-[1.75rem] border border-ink/10 bg-white/70 p-6 shadow-sm backdrop-blur transition hover:-translate-y-1 hover:shadow-soft dark:border-line-d dark:bg-paper-d/70">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="shrink-0 rounded-full bg-ink/5 px-3 py-1 text-xs font-semibold text-muted dark:bg-white/10 dark:text-muted-d">
          {project.status}
        </span>
      </div>
      ...
      <div className="mt-auto pt-6">
        {hasLink ? (
          ...
          <p className="text-sm font-semibold text-muted dark:text-muted-d">Internal / private project</p>
        )}
      </div>
    </article>
  );
